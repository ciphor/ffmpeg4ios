...
static int decode_slice_header(H264Context *h, H264Context *h0){
   ...
   /* if (slice_type == 2 || slice_type == 4 ||
    	slice_type == 7 || slice_type == 9)
    	h->s.current_picture_ptr->key_frame |= 1;
   */
    if(slice_type > 4){
        slice_type -= 5;
        ...
        case NAL_SLICE:
            ...
            if((err = decode_slice_header(hx, h)))
               break;
						/*FILE *log;
						char buf[256]={0};
						log = fopen("E:\\log.ini", "a+");
						snprintf(buf, 256, "hx->nal_unit_type: %d, hx->slice_type: %d\n", hx->nal_unit_type, hx->slice_type);
						fwrite(buf, 1, strlen(buf), log);
						fclose(log);*/
            s->current_picture_ptr->key_frame |=
                    (hx->nal_unit_type == NAL_IDR_SLICE) || 
                    ((hx->nal_unit_type == NAL_SLICE) && (hx->slice_type==FF_I_TYPE)) ||
